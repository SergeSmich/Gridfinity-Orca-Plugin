...
function triangulate(outer, holes) {
  ...
  /* ---- cap-face decimation -------------------------------------------
     Over hostile caps (plate sockets a hair from the outline and from
     each other) the ear clipper's strict blocker refuses every ear and
     the relax fan comes out covering far more than the polygon: giant
     wedges overlap each other and even spill over the socket openings
     (a thin "film" z-fighting on the plate top, and sockets closed by a
     lid).  The fan is watertight - every loop edge is covered - so only
     its INTERIOR triangles may be touched:
       * boundary triangles (owning a loop edge) are never dropped, the
         wall pairing stays exact;
       * an interior triangle with any sample inside a hole (a socket)
         covers the void -> drop;
       * an interior triangle fully covered by bigger kept ones -> drop.
     The remaining cover stays overlap-free where it matters. */
  var n2 = tris.length;
  if (n2 > 8 && hs && hs.length) {
    var isLoopEdge = new Array(n2);
    var i2, t2;
    for (i2 = 0; i2 < n2; i2++) {
      t2 = tris[i2];
      var e01 = (t2[0] + 1) % poly.length === t2[1] || (t2[1] + 1) % poly.length === t2[0];
      var e12 = (t2[1] + 1) % poly.length === t2[2] || (t2[2] + 1) % poly.length === t2[1];
      var e20 = (t2[2] + 1) % poly.length === t2[0] || (t2[0] + 1) % poly.length === t2[2];
      isLoopEdge[i2] = e01 || e12 || e20;
    }
    var ar2 = new Array(n2);
    for (i2 = 0; i2 < n2; i2++) {
      t2 = tris[i2];
      ar2[i2] = Math.abs(area2(poly[t2[0]], poly[t2[1]], poly[t2[2]])) / 2;
    }
    // sample points of a triangle: centroid + edge midpoints
    function triSamples(t3) {
    ...
    function inHolesM(px, py, m2) {
      for (var h3 = 0; h3 < hs.length; h3++) {
        ...
        for (var a3 = 0, b3 = H.length - 1; a3 < H.length; b3 = a3++) {
          if (((H[a3][1] > py) !== (H[b3][1] > py)) &&
              (px < (H[b3][0] - H[a3][0]) * (py - H[a3][1]) /
                    (H[b3][1] - H[a3][1]) + H[a3][0]))
            inside = !inside;
        }
        if (!inside) continue;
        /* only a DEEP sample counts: points hugging the hole boundary sit
           in the legitimate hairline lattice, not in the void */
        for (var a4 = 0, b4 = H.length - 1; a4 < H.length; b4 = a4++) {
          var ex = H[b4][0] - H[a4][0], ey = H[b4][1] - H[a4][1];
          var L2 = ex * ex + ey * ey;
    ...
    function coversTri(big, t3) {   // strict: every sample of t3 inside big
      ...
      for (var s3 = 0; s3 < S.length; s3++) {
        var px = S[s3][0], py = S[s3][1];
        var d1 = (v[0] - u[0]) * (py - u[1]) - (v[1] - u[1]) * (px - u[0]),
            d2 = (w[0] - v[0]) * (py - v[1]) - (w[1] - v[1]) * (px - v[0]),
            d3 = (u[0] - w[0]) * (py - w[1]) - (u[1] - w[1]) * (px - w[0]);
        if (!((d1 > 1e-12 && d2 > 1e-12 && d3 > 1e-12) ||
              (d1 < -1e-12 && d2 < -1e-12 && d3 < -1e-12))) return false;
      }
      return true;
    }
    function touchesVoid(t3) {
      var S2 = triSamples(t3);
      for (var s4 = 0; s4 < S2.length; s4++)
        if (inHoles(S2[s4][0], S2[s4][1])) return true;
      return false;
    }
    function hasEdge(t3, iA, iB) {
      return (t3[0] === iA && t3[1] === iB) || (t3[0] === iB && t3[1] === iA) ||
             (t3[1] === iA && t3[2] === iB) || (t3[1] === iB && t3[2] === iA) ||
             (t3[2] === iA && t3[0] === iB) || (t3[2] === iB && t3[0] === iA);
    }
    function coversEdge(t3, iA, iB) {   // endpoints owned + midpoint inside-or-on
      if (!hasEdge(t3, iA, iB)) return false;
      var u = poly[iA], v = poly[iB];
      var px = (u[0] + v[0]) / 2, py = (u[1] + v[1]) / 2;
      var a = poly[t3[0]], b = poly[t3[1]], c = poly[t3[2]];
      var d1 = (b[0] - a[0]) * (py - a[1]) - (b[1] - a[1]) * (px - a[0]),
...
function* plateSteps(p, seg) {
  ...
  /* a 1 um modeling epsilon: at gap = 0 the segments' seam walls would sit
     exactly on top of each other (3-4 faces per seam edge - the mesh
     checker reads that as open edges).  The epsilon keeps the pieces
     visually fused but makes every seam edge strictly manifold. */
  var gap = Math.max(0.001, p.plateGap || 0);
...
/* =====================================================================
   ...
   ===================================================================== */
/* ---- T-junction welding ---------------------------------------------
   Holed caps are triangulated by the exact scanline decomposition, whose
   boundary carries vertices interpolated along the loop edges (the slab
   event rows).  The wall bands below such a cap still run the original
   long edge, so the cap's sub-edges have no direct partners: the STL
   reads as having thousands of open edges even though the surfaces tile
   the solid exactly.  Before writing the STL we split every triangle
   edge that passes through an existing mesh vertex, so both sides of
   every edge end on the same vertices.  Purely topological - no vertex
   moves, volumes unchanged - and idempotent. */
function weldTJunctions(mesh) {
  var n = mesh.count();
  if (!n) return mesh;
  var CEL = 4, inv = 1 / CEL;
  function cellOf(v) { return Math.floor(v * inv); }
  // unique vertices (hash on 0.1 um) + coarse grid
  var vmap = new Map(), verts = [], grid = new Map(), i, t;
  for (t = 0; t < n; t++) for (i = 0; i < 3; i++) {
    var px = mesh.pos[t * 9 + i * 3], py = mesh.pos[t * 9 + i * 3 + 1], pz = mesh.pos[t * 9 + i * 3 + 2];
    var k = Math.round(px * 1e4) + "," + Math.round(py * 1e4) + "," + Math.round(pz * 1e4);
    var vi = vmap.get(k);
    if (vi === undefined) {
      vi = verts.length;
      verts.push([px, py, pz]);
      vmap.set(k, vi);
      var ck = cellOf(px) + "," + cellOf(py) + "," + cellOf(pz);
      var b = grid.get(ck);
      if (!b) { b = []; grid.set(ck, b); }
      b.push(vi);
    }
  }
  // triangles as vertex-index triples + their normals
  var tris = [], nrms = [];
  for (t = 0; t < n; t++) {
    var tri = [];
    for (i = 0; i < 3; i++) {
      var qx = mesh.pos[t * 9 + i * 3], qy = mesh.pos[t * 9 + i * 3 + 1], qz = mesh.pos[t * 9 + i * 3 + 2];
      tri.push(vmap.get(Math.round(qx * 1e4) + "," + Math.round(qy * 1e4) + "," + Math.round(qz * 1e4)));
    }
    tris.push(tri);
    nrms.push([mesh.nrm[t * 9], mesh.nrm[t * 9 + 1], mesh.nrm[t * 9 + 2]]);
  }

  function pointsOnEdge(A, B) {   // existing vertices strictly inside AB
    var ax = verts[A][0], ay = verts[A][1], az = verts[A][2];
    var bx = verts[B][0], by = verts[B][1], bz = verts[B][2];
    var ex = bx - ax, ey = by - ay, ez = bz - az;
    var L2 = ex * ex + ey * ey + ez * ez;
    if (L2 < 1e-18) return null;
    var out = null;
    var c0 = cellOf(ax), c1 = cellOf(bx),
        r0 = cellOf(ay), r1 = cellOf(by), s0 = cellOf(az), s1 = cellOf(bz);
    var steps = Math.max(Math.abs(c1 - c0), Math.abs(r1 - r0), Math.abs(s1 - s0)) + 1;
    var seen = null;
    for (var s = 0; s <= steps; s++) {
      var f = s / steps;
      var ck = cellOf(ax + ex * f) + "," + cellOf(ay + ey * f) + "," + cellOf(az + ez * f);
      if (seen === ck) continue;
      seen = ck;
      var bkt = grid.get(ck);
      if (!bkt) continue;
      for (var bi = 0; bi < bkt.length; bi++) {
        var vi = bkt[bi];
        if (vi === A || vi === B) continue;
        var wx = verts[vi][0] - ax, wy = verts[vi][1] - ay, wz = verts[vi][2] - az;
        var tt = (wx * ex + wy * ey + wz * ez) / L2;
        if (tt <= 1e-9 || tt >= 1 - 1e-9) continue;
        var dx = wx - ex * tt, dy = wy - ey * tt, dz = wz - ez * tt;
        if (dx * dx + dy * dy + dz * dz > 1e-14) continue;   // 1e-7 mm off the edge
        if (!out) out = [];
        out.push([tt, vi]);
      }
    }
    if (!out) return null;
    out.sort(function (u, v) { return u[0] - v[0]; });
    var res = [], last = -1;
    for (var oi = 0; oi < out.length; oi++)
      if (out[oi][1] !== last) { res.push(out[oi][1]); last = out[oi][1]; }
    return res;
  }

  for (var round = 0; round < 24; round++) {
    // undirected edge -> list of [triIdx, u, v] occurrences
    var emap = new Map();
    for (t = 0; t < tris.length; t++) {
      var T3 = tris[t];
      for (i = 0; i < 3; i++) {
        var a3 = T3[i], b3 = T3[(i + 1) % 3];
        var ek = a3 < b3 ? a3 + "|" + b3 : b3 + "|" + a3;
        var rec = emap.get(ek);
        if (!rec) { rec = []; emap.set(ek, rec); }
        rec.push([t, a3, b3]);
      }
    }
    // triIdx -> [u, v, pts] of its most subdivided edge
    var splits = null;
    emap.forEach(function (occ, ek) {
      var parts = ek.split("|");
      var pts = pointsOnEdge(+parts[0], +parts[1]);
      if (!pts) return;
      for (var o2 = 0; o2 < occ.length; o2++) {
        var ti = occ[o2][0];
        if (!splits) splits = new Map();
        var cur = splits.get(ti);
        if (!cur || pts.length > cur[2].length) splits.set(ti, [occ[o2][1], occ[o2][2], pts]);
      }
    });
    if (!splits) break;
    var nt2 = [];
    for (t = 0; t < tris.length; t++) {
      if (!splits.has(t)) { nt2.push(tris[t]); continue; }
      var op = splits.get(t);
      var T6 = tris[t], N6 = nrms[t];
      var bi = -1;
      for (var q4 = 0; q4 < 3; q4++)
        if ((T6[q4] === op[0] && T6[(q4 + 1) % 3] === op[1]) ||
            (T6[q4] === op[1] && T6[(q4 + 1) % 3] === op[0])) bi = q4;
      if (bi < 0) { nt2.push(T6); continue; }
      var A = T6[bi], B = T6[(bi + 1) % 3], C = T6[(bi + 2) % 3];
      /* order the chain along THIS triangle's u->v direction: the points
         were sorted along the undirected key (lower index first), which
         may be the opposite way */
      var ux = verts[A][0], uy = verts[A][1], uz = verts[A][2];
      var vx2 = verts[B][0] - ux, vy2 = verts[B][1] - uy, vz2 = verts[B][2] - uz;
      var pts6 = op[2].slice().sort(function (p1, p2) {
        var t1 = (verts[p1][0] - ux) * vx2 + (verts[p1][1] - uy) * vy2 + (verts[p1][2] - uz) * vz2;
        var t2 = (verts[p2][0] - ux) * vx2 + (verts[p2][1] - uy) * vy2 + (verts[p2][2] - uz) * vz2;
        return t1 - t2;
      });
      var chain = [A];
      for (var ci = 0; ci < pts6.length; ci++) {
        var P = pts6[ci];
        if (P !== A && P !== B) chain.push(P);
      }
      chain.push(B);
      for (var f = 0; f + 1 < chain.length; f++) {
        nt2.push([chain[f], chain[f + 1], C]);
        nrms.push(N6);
      }
    }
    tris = nt2;
  ...
  mesh.nrm = [];
  for (t = 0; t < tris.length; t++) {
    var T7 = tris[t], N7 = nrms[t];
    for (i = 0; i < 3; i++) {
      mesh.pos.push(verts[T7[i]][0], verts[T7[i]][1], verts[T7[i]][2]);
      mesh.nrm.push(N7[0], N7[1], N7[2]);
    }
  }
  return mesh;
}
...
if (typeof module !== "undefined") module.exports = { buildBin: buildBin, buildPlate: buildPlate, derive: derive, derivePlate: derivePlate, toSTL: toSTL, weldTJunctions: weldTJunctions, DEFAULTS: DEFAULTS };
