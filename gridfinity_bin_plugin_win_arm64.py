...
function ogEmitLevel(p, d, seg) {
  ...
  for (var j = 1; j < H; j++) for (var cx2 = 0; cx2 < W; cx2++) {
    ...
    stripSeg(P0, [1, 0], [0, 1]);                  // into the tile above
    stripSeg(P0, [1, 0], [0, -1]);                 // into the tile below
  }
  for (var i2 = 1; i2 < W; i2++) for (var cy2 = 0; cy2 < H; cy2++) {
    var P1 = [(i2 - W / 2) * OG.TILE, (H / 2 - cy2) * OG.TILE];
    stripSeg(P1, [0, -1], [1, 0]);                 // into the tile right
    stripSeg(P1, [0, -1], [-1, 0]);                // into the tile left
  }
...
var I18N = {
  en: {
    ...
    og_hint: "28 mm openGrid lattice; 3 OG tiles = 2 Gridfinity units. Heads wider than 7.7 mm are clamped to fit the node.",
  ...
  ru: {
    ...
    og_hint: "Решётка openGrid 28 мм; 3 тайла OG = 2 юнита Gridfinity. Головки шире 7,7 мм ограничиваются под узел.",
