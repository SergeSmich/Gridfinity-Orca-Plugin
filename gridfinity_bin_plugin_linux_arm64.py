...
<div class="app">
  ...
  <main class="stage">
    <canvas id="gl"></canvas>
    <canvas id="ov" style="position:absolute; inset:0; width:100%; height:100%; pointer-events:none" hidden></canvas>
...
function ogEmitLevel(p, d, seg) {
  ...
  function borderEdge(O, A, Sd, len) {          // A x Sd = +z; a runs 0..len
    var zs = ZS;
    ...
    cuts.push(minus(len));
    for (var ci = 0; ci + 1 < cuts.length; ci += 2) ribPiece(cuts[ci], cuts[ci + 1]);
  }
  var OX2 = d.OX / 2, OY2 = d.OY / 2;
  borderEdge([-OX2, -OY2], [1, 0], [0, 1], d.OX);        // south
  borderEdge([OX2, OY2], [-1, 0], [0, -1], d.OX);        // north
  borderEdge([-OX2, OY2], [0, -1], [1, 0], d.OY);        // west
  borderEdge([OX2, -OY2], [0, 1], [-1, 0], d.OY);        // east
  ...
  /* Sector of the node diamond owned by one remaining cell: a lofted
     right triangle from the node centre, legs on the two planes that
     separate the cell from its neighbours there, hypotenuse on the
     diamond plane -- so the pieces still abut exactly.
     fill node: world-axis quadrant (cells sit diagonally);
     edge node: half of the half-diamond, split on the perp axis. */
  function sectorTri(c, X, Y) {                 // X, Y unit boundary dirs
    return function (z) {
      var b = BAt(z);
      var pts = [[c[0], c[1]], [c[0] + X[0] * b, c[1] + X[1] * b],
                 [c[0] + Y[0] * b, c[1] + Y[1] * b]];
      if (signedArea(pts.map(function (w) { return [w[0], w[1]]; })) < 0)
        pts.reverse();
      return pts;
    };
  }
  ...
  for (var iy3 = 0; iy3 <= H; iy3++) for (var ix3 = 0; ix3 <= W; ix3++) {
    var info = ogNodeInfo(W, H, ix3, iy3);
    if (!info) continue;
    var nx = (ix3 - W / 2) * OG.TILE, ny = (H / 2 - iy3) * OG.TILE;
    var inw = info.kind === "edge"
      ? (ix3 === 0 ? [1, 0] : ix3 === W ? [-1, 0] : iy3 === 0 ? [0, -1] : [0, 1])
      : [(ix3 === 0 ? 1 : -1), (iy3 === 0 ? -1 : 1)];
    ...
    if (info.kind === "fill") {
      ...
      (function (nx, ny) {
        ...
        for (var q3 = 0; q3 < alive.length; q3++) {
          var tri = sectorTri([nx, ny], [alive[q3].dx, 0], [0, alive[q3].dy]);
          var lv3 = [];
          for (var k3 = 0; k3 < ZS.length; k3++)
            lv3.push({ loop: tri(ZS[k3]), z: ZS[k3] });
          piece(function (s) { addSolid(s, lv3, [], []); });
        }
      }
    } else if (info.kind === "outer") {         // quarter: {ID <= |x|+|y| <= B}
      if (alive.length) {
      var lvq = [];
      for (var k8 = 0; k8 < ZS.length; k8++) {
        var zq = ZS[k8], bq = BAt(zq);
        var q = [[ID, 0], [bq, 0], [bq / 2, bq / 2], [0, bq],
                 [0, ID], [ID / 2, ID / 2]];
        var pts2 = q.map(function (t) {
          return [nx + inw[0] * t[0], ny + inw[1] * t[1], zq];
        });
      ...
      if (!isConn) {
        ...
        var ppE = [-inw[1], inw[0]];
        var sE = (alive[0].dx * ppE[0] + alive[0].dy * ppE[1]) >= 0 ? 1 : -1;
        var triE = sectorTri([nx, ny], [ppE[0] * sE, ppE[1] * sE], inw);
        var lvE = [];
        for (var kE = 0; kE < ZS.length; kE++)
          lvE.push({ loop: triE(ZS[kE]), z: ZS[kE] });
        piece(function (s) { addSolid(s, lvE, [], []); });
      }
    }
  }
...
function deriveBoard(p) {
  ...
  function all4(ix, iy) {
    return !cellOff(ix - 1, iy - 1) && !cellOff(ix, iy - 1) &&
           !cellOff(ix - 1, iy) && !cellOff(ix, iy);
  }
  var snap = 0, ix, iy;
  ...
  var screws = 0;
  if (p.ogScrews && W >= 2 && H >= 2)
    for (ix = 1; ix < W; ix++) for (iy = 1; iy < H; iy++)
      if (all4(ix, iy) && !(p.ogScrewOff && p.ogScrewOff[nodeKey(ix, iy)])) screws++;
  var conns = 0;
  if (p.ogConnectors) {
    function edgeOK(ix2, iy2) {
      return all4(ix2, iy2) && !(p.ogConnOff && p.ogConnOff[nodeKey(ix2, iy2)]);
    }
    for (ix = 1; ix < W; ix++) { if (edgeOK(ix, 0)) conns++; if (edgeOK(ix, H)) conns++; }
    for (iy = 1; iy < H; iy++) { if (edgeOK(0, iy)) conns++; if (edgeOK(W, iy)) conns++; }
  }
  return {
...
var I18N = {
  en: {
    ...
    hint_2d: "2D editor: click a screw, a connector or a cell to toggle it. Drag pans, wheel zooms.",
  ...
  ru: {
    ...
    hint_2d: "Редактор 2D: клик по винту, коннектору или ячейке — удалить/вернуть. Перетаскивание — панорама, колесо — масштаб.",
...
function ogEditItems() {
  ...
  var items = { W: d.W, H: d.H, cells: [], screws: [], conns: [] };
  function off(m, k) { return !!(P[m] && P[m][k]); }
  function ok4(ix, iy) {
    return !off("ogCellOff", (ix - 1) + "," + (iy - 1)) &&
           !off("ogCellOff", ix + "," + (iy - 1)) &&
           !off("ogCellOff", (ix - 1) + "," + iy) &&
           !off("ogCellOff", ix + "," + iy);
  }
  ...
  if (P.ogScrews && d.W >= 2 && d.H >= 2)
    for (var iy = 1; iy < d.H; iy++)
      for (var ix = 1; ix < d.W; ix++)
        items.screws.push({ ix: ix, iy: iy, key: ogNodeKey((ix - d.W / 2) * T, (d.H / 2 - iy) * T),
          x: (ix - d.W / 2) * T, y: (d.H / 2 - iy) * T,
          state: !ok4(ix, iy) ? "auto"
              : (off("ogScrewOff", ogNodeKey((ix - d.W / 2) * T, (d.H / 2 - iy) * T)) ? "off" : "on") });
  if (P.ogConnectors) {
    function edge(ix, iy) {
      items.conns.push({ ix: ix, iy: iy, key: ogNodeKey((ix - d.W / 2) * T, (d.H / 2 - iy) * T),
        x: (ix - d.W / 2) * T, y: (d.H / 2 - iy) * T,
        state: !ok4(ix, iy) ? "auto"
            : (off("ogConnOff", ogNodeKey((ix - d.W / 2) * T, (d.H / 2 - iy) * T)) ? "off" : "on") });
    }
    for (var i = 1; i < d.W; i++) { edge(i, 0); edge(i, d.H); }
    for (var j = 1; j < d.H; j++) { edge(0, j); edge(d.W, j); }
  }
  return items;
}
...
function pick2DTarget(wx, wy) {
  ...
  for (i = 0; i < it.screws.length; i++) {          /* 3.5 mm */
    if (it.screws[i].state === "auto") continue;
  ...
  for (i = 0; i < it.conns.length; i++) {           /* 4.2 mm */
    if (it.conns[i].state === "auto") continue;
...
function pick2D(e) {
  ...
  if (!t) return;
  var map = t.kind === "screw" ? "ogScrewOff" : t.kind === "conn" ? "ogConnOff" : "ogCellOff";
  if (!P[map]) P[map] = {};
...
function drawOverlay() {
  ...
  for (i = 0; i < it.screws.length; i++) {
    ...
    c.beginPath(); c.arc(w2sX(e.x), w2sY(e.y), rr, 0, 6.2832);
  ...
  if (hover2D) {
    ...
    if (hover2D.kind === "cell") {
      ...
      var list = hover2D.kind === "screw" ? it.screws : it.conns;
...
function stem() {
  if (P.mode === "og") {
    ...
    if ((P.ogCellOff && Object.keys(P.ogCellOff).length) ||
        ...
        (P.ogScrewOff && Object.keys(P.ogScrewOff).length)) ogSfx += "_edit";
